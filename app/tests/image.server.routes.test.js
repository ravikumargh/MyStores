...
describe('Image CRUD tests', function() {
					...
					.end(function(imageSaveErr, imageSaveRes) {
						...
						agent.get('/images')
							.end(function(imagesGetErr, imagesGetRes) {
								...
								(images[0].title).should.match('Image Title');
	...
	it('should not be able to save an image if no title is provided', function(done) {
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/images')
					...
					.end(function(imageSaveErr, imageSaveRes) {
						...
						(imageSaveRes.body.message).should.match('Title cannot be blank');
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/images')
					...
					.end(function(imageSaveErr, imageSaveRes) {
						...
						// Update image title
						image.title = 'WHY YOU GOTTA BE SO MEAN?';
						...
						agent.put('/images/' + imageSaveRes.body._id)
							...
							.end(function(imageUpdateErr, imageUpdateRes) {
								...
								(imageUpdateRes.body.title).should.match('WHY YOU GOTTA BE SO MEAN?');
	...
	it('should be able to get a single image if not signed in', function(done) {
		...
		imageObj.save(function() {
			request(app).get('/images/' + imageObj._id)
				.end(function(req, res) {
					// Set assertion
					res.body.should.be.an.Object.with.property('title', image.title);
